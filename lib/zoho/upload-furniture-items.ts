...
export type FurnitureUploadResult = {
  attempted: number
  uploaded: number
  errors: string[]
}
...
async function workbookRows(file: File): Promise<ExcelRow[]> {
  ...
  if (!firstSheetName) {
    throw new Error(`${file.name} does not contain any sheets.`)
  }
...
export async function uploadFurnitureItemsFromExcel(
  itemFile: File,
  finishesFile: File,
): Promise<FurnitureUploadResult> {
  const [items, finishes] = await Promise.all([
    workbookRows(itemFile),
    workbookRows(finishesFile),
  ])

  if (items.length === 0) throw new Error('Items workbook has no data rows.')
