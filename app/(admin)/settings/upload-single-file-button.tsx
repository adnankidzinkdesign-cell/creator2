...
import { uploadFurnitureDataSingleFile, type UploadDataResult } from './actions'

export function UploadSingleFileButton() {
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const next = await uploadFurnitureDataSingleFile(formData)
      setResult(next)
    ...
    <form ref={formRef} action={handleSubmit} className="space-y-3">
      <div>
        <label className="space-y-1 text-sm font-medium text-tremor-content-strong dark:text-dark-tremor-content-strong">
          <span>Excel File (Items + Finishes sheets)</span>
          <input
            name="file"
            type="file"
            accept=".xlsx,.xls"
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button type="submit" loading={isPending} disabled={isPending}>
          {isPending ? 'Uploading...' : 'Upload items'}
        </Button>
        {result?.success && (
